...
@knet function cnn3(x; cwin1=2, cout1=20, hidden=100, output=20, pdrop=0.5)
	...
	return softmax_layer(h2; output=output)
end
...
function main(args)
	...
	global worldf = nothing

	if o[:chidden] == 0
		worldf = compile(:cnn; cwin1=o[:cwin], cout1=o[:cout], output=yvocab)
	elseif o[:chidden] != 0 && o[:dropout] == 0
		worldf = compile(:cnn2; cwin1=o[:cwin], cout1=o[:cout], hidden=o[:chidden], output=yvocab)
	else
		worldf = compile(:cnn3; cwin1=o[:cwin], cout1=o[:cout], output=yvocab, pdrop=o[:dropout])
	end
	...
	for epoch=1:o[:epochs]      # TODO: experiment with pretraining
		drop = o[:dropout] != 0
		@date trnerr = train(worldf, worlddata[1], softloss; gclip=o[:gclip], dropout=drop)
