...
function sparsify(data)
    colmax = maximum(data,1)
...
data = readdlm(testdir);
x_t  = sparsify(data[:,1:end-3]);
S_t  = sparsify(data[:,end-2]);
T_t  = sparsify(data[:,end-1]);
RP_t = sparsify(data[:,end]);
