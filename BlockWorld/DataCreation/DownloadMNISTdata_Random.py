...
if not os.path.exists(directory + "_random"):
  os.makedirs(directory + "_random")

j = json.load(gzip.open(directory + "_random" + ".json.gz"))
images = []
JSON = []
for file in j:
...
out = gzip.open(directory + "_random/" + directory + ".json.gz", 'w')
for file in JSON:
